...
function do_diff(loop$old, loop$old_keyed, loop$new, loop$new_keyed, loop$moved, loop$moved_offset, loop$removed, loop$node_index, loop$patch_index, loop$path, loop$changes, loop$children, loop$mapper, loop$events) {
  while (true) {
    ...
    if (old.hasLength(0) && new$8.hasLength(0)) {
      ...
      let _block$1;
      let $1 = child.patch;
      if ($1 instanceof Patch2 && $1.removed === 0 && $1.changes.hasLength(0) && $1.children.hasLength(0)) {
        _block$1 = children;
      } else {
...
var RequestData = class extends CustomType {
...
var ResponseData = class extends CustomType {
  constructor(passphrase) {
    super();
    this.passphrase = passphrase;
  }
};
function encode_request_data(data) {
...
function response_data_decoder() {
  return field(
    "passphrase",
    string2,
    (passphrase) => {
      return success(new ResponseData(passphrase));
    }
  );
}
...
var Model = class extends CustomType {
  constructor(username, password, returned_text, error) {
    super();
    this.username = username;
    this.password = password;
    this.returned_text = returned_text;
    this.error = error;
  }
};
var ServerAuthenticatedUser = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserTypedNewUsername = class extends CustomType {
  constructor(username) {
    super();
    this.username = username;
  }
};
var UserTypedNewPassword = class extends CustomType {
...
function init(_) {
  return [new Model("", "", new None(), new None()), none()];
}
function login_request(data) {
  let body = encode_request_data(data);
  let url = "/api/login";
  return post(
    url,
    body,
    expect_ok_response(
      (var0) => {
        return new ServerAuthenticatedUser(var0);
      }
    )
  );
}
function update2(model, msg) {
  if (msg instanceof ServerAuthenticatedUser && msg[0].isOk()) {
    let response = msg[0][0];
    let $ = (() => {
      let _pipe = response.body;
      return parse2(_pipe, response_data_decoder());
    })();
    if ($.isOk()) {
      let data = $[0];
      return [
      ...
      return [
        (() => {
          ...
          return new Model(
            _record.username,
            _record.password,
            _record.returned_text,
            new Some("The server did an oopsie!!")
          );
        })(),
        none()
      ];
    }
  } else if (msg instanceof ServerAuthenticatedUser && !msg[0].isOk()) {
    return [
      (() => {
        let _record = model;
        return new Model(
          ...
          _record.password,
          _record.returned_text,
          new Some("GET OUT!!! I TOLD YOU TO BUZZ OFF DIDNT I")
        );
      })(),
      none()
    ];
  } else if (msg instanceof UserTypedNewUsername) {
  ...
  } else if (msg instanceof UserTypedNewPassword) {
    ...
    return [
      ...
      login_request(new RequestData(model.username, model.password))
    ];
  }
}
function view(model) {
  return div(
    toList([]),
    toList([
      h1(toList([]), toList([text2("Buzz off")])),
      div(
        toList([]),
        toList([
          input(
            toList([
              on_input(
                (var0) => {
                  return new UserTypedNewUsername(var0);
                }
              ),
          ...
          input(
            toList([
              on_input(
                (var0) => {
                  return new UserTypedNewPassword(var0);
                }
              ),
          ...
          button(
            toList([on_click(new UserSubmittedLoginForm())]),
            toList([text2("Login")])
          )
        ])
      ),
      ...
      (() => {
        let $ = model.returned_text;
        if ($ instanceof Some) {
          let text3 = $[0];
          return h2(toList([]), toList([text2(text3)]));
        } else {
          return none2();
        }
      })()
